	...
	export default {
        ...
        methods:{
        	login(){
				...
				axios({
					method:"POST",
					url:'http://139.196.178.5:8010/ApiUser/Login',
					data:qs.stringify({
						"UserID":self.userData.UserID,
						"CodeID":self.userData.CodeID,
						"Code":self.yzm
					}),
				}).then(data =>{
					console.log(data)
			...
			sendMsg(){
				...
				axios({
					...
					if(data.data.rs){
						this.timer()
						self.userData = data.data.datas
						sessionStorage.setItem('UserID',data.data.datas.UserID)
					}
