	...
	export default {
        ...
        methods:{
			...
			setadress(){
				...
				axios({
					method:"POST",
					url:'http://139.196.178.5:8010/ApiUser/AddressCreate',
					data:qs.stringify({
						"UserID":sessionStorage.getItem('UserID'),
						"CID":sessionStorage.getItem('CID'),
						"CodeID":this.CodeID,      //验证码id
						"Code":this.code,           //验证码
						"AddType":this.select.toString(),//账号类型
						"AddMsg":this.address,         //账号地址
			...
			getcode(){
				...
				axios({
					method:"POST",
					url:'http://139.196.178.5:8010/ApiUser/AddressCreateCode',
					data:qs.stringify({
						"UserID":sessionStorage.getItem('UserID'),
						"CID":sessionStorage.getItem('CID')
					}),
					...
					this.$vux.toast.text(data.data.msg)
					if(data.data.rs){
						this.CodeID = data.data.datas.CodeID
						this.timer();
					}
				}).catch(err =>{
					console.log(err)
