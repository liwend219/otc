        ...
        methods:{
			...
			toAddaddress(){
				if(this.btnText == '+ 添加提现地址/账户'){
					...
					axios({
						method:"POST",
						url:'http://139.196.178.5:8010/ApiUser/PutCaseCreate',
						data:qs.stringify({
			...
			getadress(){
				axios({
					method:"POST",
					url:'http://139.196.178.5:8010/ApiUser/GetAddress',
					data:qs.stringify({
						"UserID":sessionStorage.getItem('UserID'),
						"CID":sessionStorage.getItem('CID'),
					}),
				}).then(data =>{
					console.log("提现地址查询",data.data)
					if(data.data.rs){
						if(data.data.datas.length == 0){
							this.txAdd = '您当前无提现地址/账户'
						}else{
							this.txAdd = '请选择您的提现地址'
							this.screen(data.data.datas)
						}
					}
				}).catch(err =>{
					console.log(err)
			...
			screen(data){
				...
				data.map(val => {
					...
					console.log((val.AddType+' - '+val.AddMsg))
				})
				this.add = [t]
