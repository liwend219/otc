...
export default {
    ...
    mounted(){
        ...
        axios({
            method:"POST",
            url:'http://139.196.178.5:8010/ApiBus/OrderDetail',
            data:qs.stringify({
            ...
            if(data.data.rs){
                ...
                this.CreateTime = parseInt(data.data.datas.CreateTime.slice(6,-2))
                this.overTime = parseInt(data.data.datas.CreateTime.slice(6,-2))+1800000
                this.screen()
                console.log(this.datas)
                if(data.data.datas == 1){
                    this.sta = false
                }else if(data.data.datas.State == 2){
                    this.sta = false
                }else if(data.data.datas.State == 3){
                    this.sta = true
                }else if(data.data.datas.State == 4){
                    this.sta = false
                }else if(data.data.datas.State == 5){
                    this.sta = true
                }else if(data.data.datas.State == 6){
                    this.sta = false
                }
            }
    ...
    methods:{
        ...
        Appeal(){
            ...
            axios({
                method:"POST",
                url:'http://139.196.178.5:8010/ApiBus/OrderServer',
                data:qs.stringify({
                    "UserID":sessionStorage.getItem('UserID'),
                    "CID":sessionStorage.getItem('CID'),
                    "OrderID":this.$route.query.id
        ...
        onConfirm(){
            axios({
                method:"POST",
                url:'http://139.196.178.5:8010/ApiBus/OrderSaleSure',
                data:qs.stringify({
                    "UserID":sessionStorage.getItem('UserID'),
                    "CID":sessionStorage.getItem('CID'),
                    "OrderID":this.$route.query.id
